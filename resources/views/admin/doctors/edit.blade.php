...
<div class="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            {{-- SEKARANG pakai $doctor->name, bukan $doctor->user->name --}}
            <h1 class="text-2xl font-bold text-teal-900">
                Edit Dokter: {{ $doctor->name }}
            </h1>
            <p class="text-sm text-gray-500">Perbarui informasi akun dan detail dokter.</p>
        </div>
    ...
    <div class="bg-white shadow-sm rounded-2xl border border-gray-100 p-6 lg:p-8">
        ...
        <form action="{{ route('admin.doctors.update', $doctor->id) }}" method="POST" class="space-y-8">
            ...
            {{-- Detail Dokter --}}
            @php
                $detail = $doctor->doctorDetail; // supaya gampang dipakai di bawah
            @endphp
            ...
            <div>
                ...
                <div class="mb-2">
                    ...
                    <textarea name="bio" id="bio" rows="3"
                              class="w-full px-3 py-2 rounded-lg border border-gray-300 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 @error('bio') border-rose-500 @enderror">{{ old('bio', $detail->bio ?? '') }}</textarea>
                    @error('bio')
                        <p class="text-rose-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
